...
consts, paramset = Simedis.constants_from_file("paramsetUkr.toml")
#disable_logging(Logging.Warn)

global victims=[]

j =JSON.parsefile("ukr.json")
nSites = 1
# coo=[]
# for i in eachindex(j)

#     append!(coo,split(j[i], " "))
...
fmp_posY=[]
home=[]
# coo=parse.(Float64,coo)


# Helper function to parse coordinates from a string
# Helper function to parse coordinates from a string and return them as an array
function parse_coordinates(coord_str)
    lat_long = split(coord_str, " ")
    return [parse(Float64, lat_long[1]), parse(Float64, lat_long[2])]
end

# Extract location coordinates, each as an array [latitude, longitude]
locations = [parse_coordinates(j[i]) for i in 1:nSites]

# Extract CCP coordinates, each as an array
ccp = [parse_coordinates(j[i]) for i in (nSites + 1):(2*nSites)]

# Extract FMP coordinates, each as an array
fmp = [parse_coordinates(j[i]) for i in (2*nSites + 1):(3*nSites)]

# Home base coordinates are assumed to be the last entry, stored as an array
for i in 1:nSites
    
    push!(home,[46.95330 , 31.94486])
end
                                            ...
                                            for HospitalDistribution in paramset.hospitaldistrList # ["SpreadOut", "CloseFirst"]
                                                ...
                                                for HospitalCapacity in paramset.hospitalcapalist # ["Low", "Medium", "High"]
                                                    ...
                                                    @time begin
                                                        ...
                                                        Simedis.reportLocations(inputDB,outputDB,m,paramset,1000,routeG,"zapo",coords,consts,0.0,0.0,true,true) # plot the html file
